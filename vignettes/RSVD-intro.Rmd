...
On the other hand, using `Rsvd` function from `rsvddpd` package enables us a mitigate the effect of this outlier.
...
Let us take a look at what `Rsvd` does under the hood. Before that, singular value decomposition (SVD) of a matrix $X$ is splitting it as; 
...
As it can be seen, the function `Rsvd` handles the very large or very small elements nicely.
...
In presence of outliers with large deviation, the performance of `Rsvd` is fairly robust to the choice of $\alpha$, the robustness parameter. With $\alpha = 0$, `Rsvd` corresponds to usual `svd` function from `base` package. However, with increasing $\alpha$, the robustness increases, i.e. even a smaller deviation would not affect the singular values, while with higher $\alpha$, the variance of the estimators generally increase.

To demonstrate the effect of $\alpha$ on time complexity, `microbenchmark` package will be used. 

```{r}
microbenchmark::microbenchmark(svd(X), Rsvd(X, alpha = 0), Rsvd(X, alpha = 0.25), 
                               Rsvd(X, alpha = 0.5), Rsvd(X, alpha = 0.75), 
                               Rsvd(X, alpha = 1), times = 30)
```
...
There are currently two packages `RobRSVD` and `pcaMethods`, which provide some implementation of a robust SVD algorithm. To compare performances of these algorithms with that of `Rsvd`, one can use `simSVD` function, which is used to simulate data matrices based on a model and then obtain an estimate of Bias and MSE of the estimates using a Monte Carlo approach.
...
```{r results='hide'}
res <- simSVD(trueSVD, svdfun = svd, B = 100, seed = 1234, outlier = TRUE, out_value = 25, tau = 0.9)
```
...
Let us see how robust svd algorithm in `pcaMethods` package performs under this simulation.

```{r results='hide'}
res <- simSVD(trueSVD, svdfun = pcaMethods::robustSvd, B = 100, seed = 1234, outlier = TRUE, out_value = 25, tau = 0.9)
```
...
`RobRSVD` package only provides the implementation of obtaining the first singular vector only. So instead of measuring performance of this, we compare `Rsvd` function's performance with the previous `svd` and `robustSvd` functions.

```{r results='hide'}
res <- simSVD(trueSVD, svdfun = Rsvd, B = 100, seed = 1234, outlier = TRUE, out_value = 25, tau = 0.9, alpha = 0.25)
```
...
```{r results='hide'}
res <- simSVD(trueSVD, svdfun = Rsvd, B = 100, seed = 1234, outlier = TRUE, out_value = 25, tau = 0.9, alpha = 0.75)
```
...
As it can be seen, the bias and MSE are much lesser in `Rsvd` algorithm.
