...
vignette: >
  %\VignetteIndexEntry{rSVDdpd-intro}
...
`RobRSVD` package only provides the implementation of obtaining the first singular vector only. So instead of measuring performance of this, we compare `rSVDdpd` function's performance with the previous `svd` and `robustSvd` functions.

```{r results='hide'}
res <- simSVD(trueSVD, svdfun = rSVDdpd, B = 100, seed = 1234, outlier = TRUE, out_value = 25, tau = 0.9, alpha = 0.25)
```
...
```{r results='hide'}
res <- simSVD(trueSVD, svdfun = rSVDdpd, B = 100, seed = 1234, outlier = TRUE, out_value = 25, tau = 0.9, alpha = 0.75)
```
